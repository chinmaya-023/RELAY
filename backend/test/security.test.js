...
import { gatewaySchema } from '../src/schemas/index.js';
...
test('gateway names are short, URL-safe, and normalized by the client before storage', () => {
  assert.deepEqual(gatewaySchema.parse({ slug: 'payments-api' }), { slug: 'payments-api' });
  for (const slug of ['ab', 'Payments-API', 'payments_api', '-payments', 'payments-']) {
    assert.throws(() => gatewaySchema.parse({ slug }), { name: 'ZodError' });
  }
});
