...
export const EmailVerification = () => {
  ...
  const [notice, setNotice] = useState('We sent a verification email when you created your account.');
  const [working, setWorking] = useState(false);
  const resend = async () => {
    setWorking(true);
    try { await sendEmailVerification(user, authActionSettings); setNotice('A new verification email has been sent. Please check your inbox and spam folder.'); }
    catch (error) { setNotice(authMessage(error, 'We could not send another verification email yet. Please wait and try again.')); }
